...
# Aim		: Finding average_relative_gain
####################################################################
	...
	while c:
		...
		for u in users:
			if u in us:
				...
				temp = (float(len(us)-us.index(u)+1)/(us.index(u)+1))
...
def temporal():
	...
	# files = ["../output/degree_cent_score.txt","../output/mcdwe_score.txt","../output/page_rank_cent.txt","../output/k_truss.txt"]

	output_file = open("../output/temp_gain1.txt","a+")
